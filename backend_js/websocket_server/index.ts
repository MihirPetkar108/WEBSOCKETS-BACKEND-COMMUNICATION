...
relaySocket.onmessage = function message({ data }) {
  const { type, roomId, name, content, host, port } = JSON.parse(
    data.toString(),
  );

  if (type === "connected") {
    console.log(`Connected to relay ${host}:${port}`);
    return;
  }
  ...
  if (type === "join") {
    if (!Rooms[roomId]) {
      ...
      console.log(`Created room ${roomId} from relay`);
    }
  ...
  if (type === "chat") {
    if (!Rooms[roomId]) {
      ...
      console.log(`Created room ${roomId} from chat relay`);
    }
...
wss.on("connection", function connection(ws) {
  ...
  ws.on("message", function messsage(data) {
    ...
    if (type === "join") {
      ...
      relaySocket.send(data.toString());
